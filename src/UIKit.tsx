      ...
      <ScrollView style={[scrollView, { backgroundColor: dark ? black : white }]}>
        ...
        <View style={{ alignItems: 'center' }}>
          ...
          <Formik
            ...
            onSubmit={(values: FormikPropType) => Alert.alert(JSON.stringify(values))}
            validationSchema={Yup.object().shape({
              email: Yup.string().email().required(),
              password: Yup.string().min(6).required()
            })}
          >
            {(props: FormikProps<FormikPropType>) => {
              const { values, handleChange, errors, setFieldTouched, touched, isValid, handleSubmit } = props
              return (
                <>
                  <Input
                    name="email"
